...
async fn test_screenshot_element() {
    ...
    let snapshot = snapshot_tool.execute(&json!({}), &mut browser).await.unwrap();
    eprintln!("Snapshot:\n{}", snapshot);

    // Extract a ref from the snapshot - look for a button or heading ref
    // The format should be like [ref=e12345]
    let ref_pattern = regex::Regex::new(r"\[ref=(e\d+)\]").unwrap();
    let captures: Vec<_> = ref_pattern.captures_iter(&snapshot).collect();
    assert!(!captures.is_empty(), "Should find at least one ref in snapshot");
    ...
    assert!(result.is_ok(), "Element screenshot failed: {:?}", result.err());
...
async fn test_element_screenshot_bounding_box_workaround() {
    ...
    // Get snapshot refs
    let snapshot = page.aria_snapshot_with_frames().await.expect("Snapshot failed");
